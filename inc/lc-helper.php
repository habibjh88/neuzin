...
class Helper {
	...
	public function theme_menu() {
		add_theme_page( esc_html__( 'Theme License', 'neuzin' ), esc_html__( 'Theme License', 'neuzin' ), 'manage_options', 'rtlc', [ $this, 'create_admin_page' ], null, 99 );
	}
	...
	public function create_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Theme License', 'neuzin' ); ?></h1>
			<form method="post" action="options.php">
			<?php
				// This prints out all hidden setting fields.
				settings_fields( 'rt_option_group' );
				do_settings_sections( 'neuzin-setting' );
			?>
			</form>
		</div>
		<?php
	...
	public function theme_option() {
		...
		add_settings_field(
			'rt_purchase_code',
			esc_html__( 'Purchase Code', 'neuzin' ),
		...
		add_settings_field(
			'rt_license_status',
			esc_html__( 'License Status', 'neuzin' ),
	...
	public function license_status_callback() {
		...
		$status_text = esc_html__( 'Not Activated', 'neuzin' );

		if ( rtlc_is_valid()['success'] ) {
			$verify      = true;
			$status_text = __( 'Activated', 'neuzin' );
		} elseif ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$status_text = __( 'Domain Mismatch', 'neuzin' );
		}
	...
	public function license_check_callback() {
		printf(
			...
			esc_html__( 'Activate License', 'neuzin' )
		);
	}
	...
	public function rtlc_verification() {
		...
		if ( $purchase_code ) {
			...
			$api_url     = "{$rt_license_server}/?theme_name={$neuzin}&purchase_code=" . $purchase_code . '&domain_name=' . $domain_name;
