...
if ( !class_exists( 'NeuzinTheme_Helper' ) ) {
	
	class NeuzinTheme_Helper {
		...
		public static function socials(){
			$neuzin_socials = array(
				'social_facebook' => array(
					'icon' => 'fa-facebook-f',
					'url'  => NeuzinTheme::neuzin_options('social_facebook'),
				),
				'social_twitter' => array(
					'icon' => 'fa-twitter',
					'url'  => NeuzinTheme::neuzin_options('social_twitter'),
				),
				'social_gplus' => array(
					'icon' => 'fa-google-plus-g',
					'url'  => NeuzinTheme::neuzin_options('social_gplus'),
				),
				'social_behance' => array(
					'icon' => 'fa-behance',
					'url'  => NeuzinTheme::neuzin_options('social_behance'),
				),
				'social_dribbble' => array(
					'icon' => 'fa-dribbble',
					'url'  => NeuzinTheme::neuzin_options('social_dribbble'),
				),
				'social_linkedin' => array(
					'icon' => 'fa-linkedin-in',
					'url'  => NeuzinTheme::neuzin_options('social_linkedin'),
				),
				'social_youtube' => array(
					'icon' => 'fa-youtube',
					'url'  => NeuzinTheme::neuzin_options('social_youtube'),
				),
				'social_pinterest' => array(
					'icon' => 'fa-pinterest-p',
					'url'  => NeuzinTheme::neuzin_options('social_pinterest'),
				),
				'social_instagram' => array(
					'icon' => 'fa-instagram',
					'url'  => NeuzinTheme::neuzin_options('social_instagram'),
				),
				'social_skype' => array(
					'icon' => 'fa-skype',
					'url'  => NeuzinTheme::neuzin_options('social_skype'),
				),
				'social_rss' => array(
					'icon' => 'fas fa-rss',
					'url'  => NeuzinTheme::neuzin_options('social_rss'),
				),
				'social_snapchat' => array(
					'icon' => 'fa-snapchat-ghost',
					'url'  => NeuzinTheme::neuzin_options('social_snapchat'),
				),
			);
			return array_filter( $neuzin_socials, array( 'NeuzinTheme_Helper' , 'filter_social' ) );
		}
		public static function team_socials(){
			...
			return apply_filters( 'NeuzinTheme_Helper', $team_socials );
		}
		...
		public static function has_footer(){
			...
			if ( NeuzinTheme::$footer_style == 2  ) {
				$footer_column = NeuzinTheme::neuzin_options('footer_column_2');
				...
				return false;
			} else if ( NeuzinTheme::$footer_style == 3 ) {
				$footer_column = NeuzinTheme::neuzin_options('footer_column_3');
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;				
			} else if ( NeuzinTheme::$footer_style == 4 ) {
				$footer_column = NeuzinTheme::neuzin_options('footer_column_4');
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;
			} else if ( NeuzinTheme::$footer_style == 5 ) {
				$footer_column = NeuzinTheme::neuzin_options('footer_column_5');
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
		...
		public static function has_active_widget(){
			...
			if ( ( class_exists( 'WooCommerce' ) && is_shop() ) || ( class_exists( 'WooCommerce' ) && is_product_category() ) || ( class_exists( 'WooCommerce' ) && is_product_tag() ) || ( class_exists( 'WooCommerce' ) && is_product() ) ) {	
				...
				if ( NeuzinTheme::$sidebar ) {
					if ( is_active_sidebar( NeuzinTheme::$sidebar ) ) {
						$neuzin_layout_class = 'col-lg-8 col-md-12 col-12';
					} else {
						$neuzin_layout_class = 'col-sm-12 col-12';
					}
